...
public class NoticeDao {
	...
	public List<Notice> selectTop3List() {
		...
		try {
			...
			list=session.selectList("notice.selectTop3List");
	...
	Notice  noti= null;
		...
		try {
			session=new SqlSessionFactoryBuilder().build(Resources.getResourceAsStream("myBatis/mybatis-config.xml")).openSession(false);
			noti=session.selectOne("notice.selectNotice",noticeNo);
	...
	public int insertNotice(Notice notice) {
		...
		try {
			session=new SqlSessionFactoryBuilder().build(Resources.getResourceAsStream("myBatis/mybatis-config.xml")).openSession(false);
			result=session.selectOne("notice.insertNotice",notice);
	...
	public int updateNotice(Notice notice) {
		...
		try {
			session=new SqlSessionFactoryBuilder().build(Resources.getResourceAsStream("myBatis/mybatis-config.xml")).openSession(false);
			result=session.selectOne("notice.updateNotice",notice);
